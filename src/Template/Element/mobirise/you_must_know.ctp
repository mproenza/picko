<?php use \Cake\I18n\I18n;?>
<h3 class="mbr-section-title pb-3 align-center mbr-fonts-style display-2">
    <?php echo __d('/mobirise/home', 'Cosas que debes tener en cuenta si vas a usar nuestros servicios')?>
</h3>
<div class="row justify-content-center">
    <div class="cards-block">
        <div class="cards-container">
            <div class="card px-3 align-left col-12">
                <div class="panel-item p-4 d-flex">
                    <!--<div class="card-img pr-3">
                        <span class="img-text d-flex align-items-center justify-content-center">
                            2
                        </span>
                    </div>-->
                    <div class="card-text">
                        <h4 class="mbr-content-title mbr-bold mbr-fonts-style display-6"><?= __d('home', 'No hacemos recogidas en los aeropuertos')?></h4>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?= __d('home', 'Debido a los horarios fijos de nuestros taxis, es muy difícil hacer que las personas lleguen y salgan del aeropuerto en el momento apropiado.')?>
                        </p>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?= __d('home', 'Sin embargo, sí aceptamos aeropuertos como lugar de destino <em>-ej. si vas de Trinidad al aeropuerto de La Habana</em>.')?>
                        </p>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <em><?= __d('home', 'Ej. Si necesitas ir del aeropuerto de La Habana directamente a Viñales, no podemos hacerlo.')?></em>
                        </p>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?php
                            $lang = I18n::getLocale();
                            if($lang == 'es') $articleUrl = 'https://pickocar.com/es/blog/desde-el-aeropuerto-de-la-habana-hasta-trinidad-varadero-vinales-y-otros-destinos-en-cuba/';
                            else $articleUrl = 'https://pickocar.com/en/blog/from-havana-airport-to-trinidad-varadero-vinales-and-other-destinations-in-cuba/';
                            ?>
                            <b><?= __('Lee')?>:</b> 
                            <a href="<?= $articleUrl?>">
                                <?php echo __d('home', '¿Cómo ir directo desde el aeropuerto de La Habana a Trinidad, Viñales, Varadero y otros destinos en Cuba?')?>
                            </a>
                        </p>
                    </div>
                </div>
            </div>
            <div class="card px-3 align-left col-12">
                <div class="panel-item p-4 d-flex">
                    <!--<div class="card-img pr-3">
                        <span class="img-text d-flex align-items-center justify-content-center">
                            3
                        </span>
                    </div>-->
                    <div class="card-text">
                        <h4 class="mbr-content-title mbr-bold mbr-fonts-style display-6">
                            <?php echo __d('home', 'La recogida (en casa u hotel) puede demorar hasta 30 minutos después del horario establecido')?>
                        </h4>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?php echo __d('home', 'Esto se debe a que tenemos que recoger pasajeros en diferentes lugares (normalmente en dos lugares diferentes). El taxi recogerá a unos pasajeros antes que otros, y luego partirá hacia el destino.')?>
                        </p>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <em><?php echo __d('home', 'Ej. Si reservas para ir de La Habana a Varadero a las 8 am, el taxi te pudiera recoger a las 8:30 am.')?></em>
                        </p>
                    </div>
                </div>
            </div>
            <div class="card px-3 align-left col-12">
                <div class="panel-item p-4 d-flex">
                    <!--<div class="card-img pr-3">
                        <span class="img-text d-flex align-items-center justify-content-center">
                            1
                        </span>
                    </div>-->
                    <div class="card-text">
                        <h4 class="mbr-content-title mbr-bold mbr-fonts-style display-6">
                            <?php echo __d('home', 'Los horarios de salida de nuestros taxis son fijos y no pueden cambiarse')?>
                        </h4>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?php echo __d('home', 'Si necesitas salir hacia tu destino en un horario diferente a los que ofrecemos, entonces no podemos atender tu necesidad.')?>
                        </p>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <em><?php echo __d('home', 'Ej. Si quieres ir de Varadero a La Habana a las 10 am, no podemos hacerlo porque ese horario no lo ofrecemos.')?></em>
                        </p>
                    </div>
                </div>
            </div>
            <div class="card px-3 align-left col-12">
                <div class="panel-item p-4 d-flex">
                    <!--<div class="card-img pr-3">
                        <span class="img-text d-flex align-items-center justify-content-center">
                            4
                        </span>
                    </div>-->
                    <div class="card-texts">
                        <h4 class="mbr-content-title mbr-bold mbr-fonts-style display-6">
                            <?php echo __d('home', 'No realizamos paradas de tipo excursionista durante los viajes')?>
                        </h4>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?php echo __d('home', 'Esto se debe a que el auto debe llegar a su destino a una hora adecuada para prestar otro servicio.')?>
                        </p>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?php echo __d('home', 'En los tramos largos (<em>ej. La Habana - Trinidad</em>) realizamos una parada en cafetería para merendar e ir al baño, y además se pueden solicitar otras paradas para cualquier otra necesidad.')?>
                        </p>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <em><?php echo __d('home', 'Ej. En un viaje de La Habana a Trinidad, no hacemos parada para visitar Cienfuegos.')?></em>
                        </p>
                        <br/>
                        <p class="mbr-content-text mbr-fonts-style display-7">
                            <?php
                            $lang = I18n::getLocale();
                            if($lang == 'es') $articleUrl = 'https://pickocar.com/es/blog/la-habana-a-trinidad-pasando-por-cienfuegos-en-transporte-economico/';
                            else $articleUrl = 'https://pickocar.com/en/blog/from-havana-to-trinidad-visiting-cienfuegos-affordable-transportation/';
                            ?>
                            <b><?= __('Lee')?>:</b> 
                            <a href="<?= $articleUrl?>">
                                <?php echo __d('home', '¿Cómo ir de La Habana a Trinidad pasando por Cienfuegos el mismo día?')?>
                            </a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>